...
window.switchTab = function(tabName) {
  ...
  const titles = {
    ...
    'wallets': 'Wallet Ledger & Control',
    'dataplans': 'Data Plans & Pricing'
  };
...
async function loadUsers() {
  const { data: users, error } = await supabase
    .from("users")
    .select("id, email, phone, first_name, last_name, merchant_id, role, wallet_balance, created_at")
    .order("created_at", { ascending: false });

  if (!error && users) allUsersCache = users;
    ...
    tbody.innerHTML += `
      <tr>
        <td style="white-space:nowrap;">
          ...
          ${u.phone ? `<div style="font-size:11px; color:#64748b; margin-top:2px;">📞 ${u.phone}</div>` : ''}
        </td>
        <td style="white-space:nowrap;"><span style="background:${roleColor}22; color:${roleColor}; font-weight:700; font-size:11px; text-transform:uppercase; letter-spacing:1px; padding:4px 10px; border-radius:20px; border:1px solid ${roleColor}44;">${u.role}</span></td>
        <td style="white-space:nowrap;"><strong style="font-size:16px;">₵${Number(u.wallet_balance || 0).toFixed(2)}</strong></td>
        <td style="white-space:nowrap;">
           <button class="btn-action" onclick="openUserWalletModal('${u.id}', ${u.wallet_balance})">± Bank</button>
...
async function loadRecentOrders(page = 1) {
  ...
  if (error) {
    ...
    tbody.innerHTML = `<tr><td colspan="7" style="text-align:center; color:#ef4444; padding:24px;">Failed to load orders: <b>${error.message}</b><br><br><i>Did you run the add_order_number.sql script in Supabase?</i></td></tr>`;
    return;
  }
  ...
  orders.forEach(order => {
    ...
    tbody.innerHTML += `
      <tr>
        ...
        <td style="font-family:monospace; font-size:11px; color:var(--blue); font-weight:700; white-space:nowrap;">${(order.network || 'ORD').toUpperCase()}-${String(order.order_number || 0).padStart(3, '0')}</td>
...
async function loadWalletTransactions() {
    ...
    const { data, error } = await supabase.from('transactions')
        .select('id, type, amount, balance_after, created_at, users(email)')
        .order('created_at', { ascending: false })
        .limit(50);
    ...
    tbody.innerHTML = '';
    data.forEach(tx => {
        const d = new Date(tx.created_at).toLocaleString('en-GB', {day:'2-digit', month:'short', hour:'2-digit', minute:'2-digit'});
        const typeColor = tx.type.toLowerCase().includes('debit') ? '#ef4444' : '#10b981';
        tbody.innerHTML += `
            <tr>
                <td style="color:var(--text-muted); font-size:12px;">${d}</td>
                <td style="font-weight:600;">${tx.users?.email || 'Unknown'}</td>
        ...
        afaData.forEach(r => {
            ...
            tbody.innerHTML += `
                <tr>
                    ...
                    <td style="white-space:nowrap;">
                        <div style="font-weight:600; color:white;">${escapeQuote(r.full_name)}</div>
...
async function loadGateways() {
    try {
        const { data: settings, error } = await supabase
            .from('settings')
            .select('key, value')
            .in('key', ['paystack_enabled', 'paystack_public_key', 'manual_transfer_enabled', 'manual_momo_number', 'manual_momo_name']);
...
async function loadTopupRequests() {
    ...
    try {
        const { data: txs, error } = await supabase
            .from('transactions')
            .select('id, user_id, amount, status, reference, created_at, users(first_name, last_name, email, phone)')
            .ilike('type', '%Deposit%')
            .order('created_at', { ascending: false })
            .limit(500);
...
function renderTopupPage() {
    ...
    pageTxs.forEach(tx => {
        ...
        tbody.innerHTML += `
            <tr>
                <td style="white-space:nowrap; color:var(--text-muted); font-size:12px;">${date}</td>
                <td style="white-space:nowrap;">
...
window.filterTopupSearch = function() {
    ...
    if (!q) {
        ...
        window.allTopupData = window.allTopupDataFull.filter(tx => {
            ...
            // merchant_id isn't fetched; use email as fallback identifier
            const email  = (tx.users?.email || '').toLowerCase();
            const name   = (`${tx.users?.first_name || ''} ${tx.users?.last_name || ''}`).toLowerCase();
            return phone.includes(q) || ref.includes(q) || email.includes(q) || name.includes(q);
        });
    }
    currentTopupPage = 1;
    renderTopupPage();
...
function renderTicketsTable(tickets) {
    ...
    tickets.forEach(t => {
        ...
        tbody.innerHTML += `
            <tr>
                <td style="color:var(--text-muted); font-size:12px; white-space:nowrap;">${d}</td>
                <td style="white-space:nowrap;">
...
window.openTicketReviewModal = function(ticketId) {
    ...
    if (ticket.order_id) {
        orderLink.innerText = `#ORD-${ticket.order_id.split('-')[0].toUpperCase()}`;
        orderLink.style.display = 'inline';
        orderLink.onclick = () => { 
            document.getElementById('ticketSearch').value = `#ORD-${ticket.order_id.split('-')[0].toUpperCase()}`;
...
window.loadEcardInventory = async function() {
    ...
    try {
        ...
        tbody.innerHTML = data.map((r, i) => {
            ...
            const usedAt = r.used_at ? new Date(r.used_at).toLocaleDateString('en-GB', { day:'2-digit', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit' }) : '—';
            return `
                <tr>
...
async function loadUsers() {
    ...
    tbody.innerHTML = `<tr><td colspan="4" style="text-align:center; padding:24px; color:var(--text-muted);">Loading users...</td></tr>`;
    ...
    try {
        ...
        tbody.innerHTML = `<tr><td colspan="4" style="text-align:center; padding:24px; color:#ef4444;">Failed to load users.</td></tr>`;
    }
}
...
function renderUsersTable() {
    ...
    if (data.length === 0) {
        tbody.innerHTML = `<tr><td colspan="4" style="text-align:center; padding:24px; color:var(--text-muted);">No users found.</td></tr>`;
        return;
    }
    ...
    tbody.innerHTML = data.map(u => {
        ...
        return `
            <tr>
                ...
                <td style="white-space:nowrap;"><strong style="font-size:15px; color:${bal > 0 ? '#10b981' : 'white'};">₵${bal}</strong></td>
    ...
    try {
        ...
        if (!orders || orders.length === 0) {
            ...
            oBody.innerHTML = orders.map(o => {
                const date = new Date(o.created_at).toLocaleString('en-GB', {day:'2-digit', month:'short', hour:'2-digit', minute:'2-digit'});
                ...
                return `
                    <tr>
                        ...
                        <td style="font-family:monospace; color:#3b82f6; white-space:nowrap;">#${String(o.id).split('-')[0].toUpperCase()}</td>
