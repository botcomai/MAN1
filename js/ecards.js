...
async function loadHistory() {
    try {
        ...
        tbody.innerHTML = orders.map(o => {
            ...
            const pinInfo = o.ecard_pin ? `<div style="font-size:11px; color:#94a3b8; margin-top:3px;">PIN: <code>${o.ecard_pin}</code></div>` : '';
            return `
                <tr>
                    <td>${date}</td>
                    <td>${typeLabel}${pinInfo}</td>
...
window.submitEcard = async function() {
    ...
    try {
        // 1. Find an available (unused) PIN
        ...
        if (!pins || pins.length === 0) {
            throw new Error('No PINs available for this product. Please contact support.');
        }
        ...
        await supabase.from('transactions').insert([{
            ...
            type: 'debit',
            amount: price,
            description: `${selectedProduct.toUpperCase()} E-Card PIN dispensed`,
            status: 'completed'
        }]);
        ...
        showPinResult(pinRecord.pin, pinRecord.serial, selectedProduct);
